...
-- Leaves will be Raw or Modified-Raw 
let Tree : Type -> Type
 = \(A: Type)
 -> forall(Tree: Type -> Type)
 -> forall(Leaf: A -> Tree A)
 -> forall(Node: Tree A -> Tree A -> Tree A)
 -> Tree A
...
let SomeRawIO =
  ...
  -> (   forall(I: Type)
      -> forall(O: Type)
      -> { toRequest:    I -> Request,
           fromResponse: Response -> Optional O
	 }
      -> Y
  )
  -> Y
...
let RawValue = \(I: Type) -> \(O: Type) -> { name : Text, toReq: I -> Request, fromResp: Response -> O }
...
let rawTypes2
  : SomeRaw2 -> { inputType : Type, outputType : Type }
  =  \(api : SomeRaw2)
  -> let f = \(I : Type)
          -> \(O : Type)
	  -> \(r: {name : Text,
	  ...
	  -> {j = {inputType = I, outputType = O}, k = +1 }
     in (api {inputType : Type, outputType : Type} (Integer) f).j
...
-- 		   fromResp : Response -> O
-- 		  })
-- 	  -> {j = {inputType = I, outputType = O},
-- 	      k = { name = r.name, fromResponse = \(x: { body : Text}) -> r.fromResp x }
--              }
--      let tys = rawTypes2 api
--      let i   = tys.inputType
--      let o   = tys.outputType

--      in (api {inputType : Type, outputType : Type}
--              { name : Text,
--      	       fromResponse : { body: Text} -> o }
--      	     f).j
...
-- 		  }
--               )
-- 	  -> {j = {inputType = r.inputType, outputType = r.outputType},
-- 	      k = { toRequests = r.toReq,
-- 	            fromResponse = r.fromResp
-- 	          }
-- 	     }
--     let tys = rawTypes2 api : { inputType : Type, outputType : Type }

--     in (api { inputType : Type, outputType : Type }
--             { toRequests : tys.inputType -> Request,
-- 	      fromResponse : Response -> tys.outputType
-- 	    }
-- 	    f).k
